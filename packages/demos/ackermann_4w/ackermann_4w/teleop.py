"""ROS2 teleop + synthetic odom/scan for `ackermann_4w`.

When run via `ros2 launch ackermann_4w sim.launch.py` (Docker/colcon), this node
...
def main() -> None:
    try:
        ...
        print(
            "[ackermann_4w] rclpy not available — use Lappa native sim offline, "
            "or Docker: lappa docker launch --demo ackermann_4w"
        )
        return
    ...
    class TeleopNode(Node):
        def __init__(self) -> None:
            ...
            self.kind = "ackermann"
            self.snapshot = SnapshotWriter("ackermann_4w")
            self.x = 0.0
            self.y = 0.0
            self.th = 0.0
            self.cmd = Twist()
            ...
            self.get_logger().info(
                f"[ackermann_4w] ROS2 node up · kind={self.kind} · /cmd_vel → /odom /scan"
            )
