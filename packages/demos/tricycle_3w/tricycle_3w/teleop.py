"""ROS2 teleop + synthetic odom/scan for `tricycle_3w`.

When run via `ros2 launch tricycle_3w sim.launch.py` (Docker/colcon), this node
    ...
    class TeleopNode(Node):
        def __init__(self) -> None:
            ...
            self.get_logger().info(
                f"[tricycle_3w] ROS2 node up · kind={self.kind} · /cmd_vel → /odom /scan"
            )
